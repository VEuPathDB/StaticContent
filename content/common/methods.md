...
<p>The identification of orthologs is an important concept in gene evolution, commonly used to infer likely gene function(s) in newly-sequenced genomes.  Orthologs are proteins in different species that have diverged solely through speciation, and are therefore relatively likely to retain similar function (<a href="https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5877793/" target="_blank">Gabaldón & Koonin 2013</a>).  Paralogs are proteins that arose through gene duplication; while initially identical, duplication provides the opportunity for functional divergence and evolutionary selection.  “In-paralogs” refer to recent duplications that are likely to retain similar function; “out-paralogs” derive from more ancient duplications predating divergence of the species of interest, and are more likely to have diverged in function.  See <a href="https://www.ncbi.nlm.nih.gov/pmc/articles/PMC1849888/" target="_blank">Chen 2007</a> and the <a href="https://orthology.benchmarkservice.org/proxy/" target="_blank">Orthology Benchmarking Service (Altenhoff 2020)</a> for descriptions and assessments of widely-used ortholog detection algorithms</p>
  ...
  <h4><a href="https://orthomcl.org/" target="_blank">OrthoMCL</a></h4>
  <div class="method-details">
  ...
  <div class="method-details">
   <p>Millions of genes have yet to be assigned a function. Gene function can be inferred using sequence similarity since genes that share sequence are likely to share three-dimensional structure and therefore biochemical activity. In order to predict gene function and thus stimulate biological discovery, VEuPathDB infers EC (enzyme classification) number assignment using orthology and protein feature information. Our <a href="https://orthomcl.org" target="_blank">OrthoMCL website</a> sorts orthologs and paralogs into ortholog groups based on sequence similarity. Proteins in these groups share overall function, but might differ slightly in aspects of protein function, such as substrate or product specificity. Thus, an ortholog group might contain some proteins with one EC number (such as 2.7.4.3) as well as other proteins with a slightly different EC number (such 2.7.4.14). In order to accurately assign each protein an EC number, VEuPathDB assesses each protein for three features (length, sequence similarity using BLAST, and domain architecture) relative to proteins in its OrthoMCL group that have already been assigned that EC number. This assessment results in three scores, one for each feature, allowing the researcher to assess confidence in this prediction. In addition, other statistics about the group, such as how many other proteins and genera share this EC number, are included. See more details about the prediction methods in <a href="{{'/documents/Inferred_EC_numbers_algorithm_document.pdf' | absolute_url }}">this document</a>. The prediction, scores and statistics are shown on the gene page in the 'Function prediction' section. If a gene is assigned an EC number, relevant rows will be added to the 'Metabolic Pathways' and 'Metabolic Pathways Reactions' tables in the  'Pathways and interactions' section of the gene page.</p>
   </div>
